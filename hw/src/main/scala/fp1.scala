...
object fp1:
  ...
  def fib(n: Int): Int =
    if n < 0 then throw UnsupportedOperationException()
  ...
  def swap(p: (Int, String)): (String, Int) =
    if p.equals(null) then throw UnsupportedOperationException()
    else (p._2, p._1)
  ...
  def maxTailAux(xs: List[Int], accumulator: Int): Int =
    ...
    else if xs.length == 1 then (accumulator max xs.head)
